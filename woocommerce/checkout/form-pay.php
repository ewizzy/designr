	...
	<table class="shop_table">
		<thead>
			<tr>
				...
				<th class="product-quantity"><?php _e( 'Qty', 'designr' ); ?></th>
				<th class="product-total"><?php _e( 'Totals', 'designr' ); ?></th>
			</tr>
		</thead>
		<tbody>
			<?php if ( sizeof( $order->get_items() ) > 0 ) : ?>
				<?php foreach ( $order->get_items() as $item_id => $item ) : ?>
					...
					<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_order_item_class', 'order_item', $item, $order ) ); ?>">
						...
						<td class="product-subtotal"><?php echo wp_filter_post_kses($order->get_formatted_line_subtotal( $item )); ?></td>
					</tr>
				<?php endforeach; ?>
			<?php endif; ?>
		</tbody>
