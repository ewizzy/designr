...
<form class="cart" method="post" enctype='multipart/form-data'>
			...
			<?php
				...
				foreach ( $grouped_products as $grouped_product ) {
					...
					<tr id="product-<?php the_ID(); ?>" <?php post_class(); ?>>
						...
						<td class="label">
							<label for="product-<?php echo esc_html($grouped_product->get_id()); ?>">
								<?php echo esc_html($grouped_product->is_visible()) ? '<a href="' . esc_url( apply_filters( 'woocommerce_grouped_product_list_link', get_permalink( $grouped_product->get_id() ), $grouped_product->get_id() ) ) . '">' . $grouped_product->get_name() . '</a>' : $grouped_product->get_name(); ?>
							</label>
						</td>
						<?php do_action( 'woocommerce_grouped_product_list_before_price', $grouped_product ); ?>
						<td class="price">
							<?php
								echo wp_filter_post_kses($grouped_product->get_price_html());
	...
	<?php if ( $quantites_required ) : ?>
		...
		<button type="submit" class="btn-default single_add_to_cart_button"><?php echo esc_html( $product->single_add_to_cart_text() ); ?></button>
